...
do
    ...
    echo 'Creating repository: vuln-images/'$image_base_name
        aws ecr create-repository --repository-name vuln-images/$image_base_name --region ap-northeast-1 --tags Key=inspector-demo,Value=true

        # now upload the image to ECR

    echo 'Tagging image'
        docker tag $i $account.dkr.ecr.ap-northeast-1.amazonaws.com/vuln-images/$image_base_name:latest

        echo 'Pushing image to ECR'
        docker push $account.dkr.ecr.ap-northeast-1.amazonaws.com/vuln-images/$image_base_name:latest
